namespace Calculator
{
    public partial class Calculator : Form
    {
        ...
        private Rectangle displayLabelButtonOriginalRectangle;
        private Size originalFormSize; //keep track of form size
        ...
        private void Calculator_Load(object sender, EventArgs e) //when thr form loads, this is whats being used
        {
            originalFormSize = this.Size;  //saying this is going to be the size

            //(ref to the x, y loc, then width and height ) for resizing
            ...
            negativeButtonOriginalRectangle = new Rectangle(plusMinusButton.Location.X, plusMinusButton.Location.Y, plusMinusButton.Width, plusMinusButton.Height);
            ...
            squareButtonOriginalRectangle = new Rectangle(sqButton.Location.X, sqButton.Location.Y, sqButton.Width, sqButton.Height);
            ...
            displayLabelButtonOriginalRectangle = new Rectangle(displayLabel.Location.X, displayLabel.Location.Y, displayLabel.Width, displayLabel.Height); 

        }

        private void Calculator_Resize(object sender, EventArgs e)
        {
            //when user resizes, this will be called and resize the size of the button
            ...
            resizeControl(negativeButtonOriginalRectangle, plusMinusButton); 
            resizeControl(clearButtonOriginalRectangle, clearbutton); 
            resizeControl(decimalButtonOriginalRectangle, decimalButton); 
            resizeControl(squareButtonOriginalRectangle, sqButton); 
            ...
            resizeControl(displayLabelButtonOriginalRectangle, displayLabel); 
        }
        private void resizeControl(Rectangle OriginalControlRect, Control control) //handle the resize and keep in the same spot
        {
            ...
            control.Location = new Point(newXPosition, newYPosition); //change the location of the control
        ...
        private void changeLabel(int numPressed)
        {
            if(dec == true)
            {
                int decimalcount = 0; //making sure the user cant enter multiple decimal points
        ...
        {
            displayLabel.Text = "-" + displayLabel.Text; //make current number in view, negative
        }
        ...
        private void backspaceButton_Click(object sender, EventArgs e)
        {
            ...
            if(stringLength > 1)
            {
                //taking the first number and the last number then taking away the last numbr
